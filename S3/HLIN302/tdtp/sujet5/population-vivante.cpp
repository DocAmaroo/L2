...
#define CHECK_BOUND(i,j) if (i>=N || j>=N){	std::cout<<"Accessing a Cell at ("<<i<<","<<j<<") out of range ... aborting"<<std::endl;std::terminate();}
...
PopulationVivante::PopulationVivante(size_t n,float p) : alive(0), N(n), probabilite(p) {
  if (N*N > NMAX) {
    std::cout<<"ATTENTION: Tableau de taille insuffisante dans PopulationVivante"<<std::endl;
  }
  if ( p > 1 || p < 0){
    std::cout<<"ATTENTION: La probabilité est soit supérieure a 1, ou inférieure à 0"<<std::endl;
  }
  regen();
}
...
size_t PopulationVivante::get_dimension() const { return N;}
float PopulationVivante::get_probabilite() const { return probabilite;}
// Accesseurs écriture
void PopulationVivante::set_probabilite(float prob){
  probabilite = prob;
  regen();
}
void PopulationVivante::set_dimension(size_t d){
  N = d;
  regen();
}
...
void PopulationVivante::regen(){
  srand(time(NULL));
  alive = 0;
  for (size_t i = 0 ; i < N ; i++) {
    for (size_t j = 0 ; j < N ; j++){
      if ( (rand() % 10000000) <= 10000000.0 * probabilite){
        birth(i,j);
      }
    }
  }
